...
char aoc24_04::neighbour_ch(const Grid& grid, size_t x, size_t y, Direction dir)
{
	Offset offs{dir_to_xy(dir)};
	return grid.char_at(x + offs.x, y + offs.y);
}

int aoc24_04::count_x_mas(const Grid& g)
{
	int n = 0;
	XY xy{0, 0};
	while (xy = g.find_char('A', xy.x, xy.y), xy != XY::oob) {
