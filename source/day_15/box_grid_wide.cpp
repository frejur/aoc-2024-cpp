...
bool has_cached_pos(const aoc24_15::Vec2d& pos,
               ...
               std::move(dyn_bitset))
{
	...
	for (Box& b : box_) {
		if (b.position().x > 0 && !has_cached_l(b.position(), cache_updated)) {
			Box_grid_wide::update_adj(b, Direction::Left);
		}
		if (b.position().y > 0 && !has_cached_u(b.position(), cache_updated)) {
			Box_grid_wide::update_adj(b, Direction::Up);
		}
		Box_grid_wide::update_adj(b, Direction::Right);
		Box_grid_wide::update_adj(b, Direction::Down);
...
void aoc24_15::Box_grid_wide::move_robot(Direction dir)
{
	...
	if (walls_map().test(adj_pos) || stuck_map(Direction::Left).test(adj_pos)
	    || stuck_map(Direction::Right).test(adj_pos)) {
		return;
	} else if (movable_map(Direction::Left).test(adj_pos)
	           || movable_map(Direction::Right).test(adj_pos)) {
		...
		if (move_boxes) {
			...
			Vec2d xy_L = {adj_xy.x + offs_L.x, adj_xy.y + offs_L.y};
			size_t pos_L = static_cast<size_t>(xy_L.y * sz + xy_L.x);
			if (movable_map(Direction::Right).test(pos_L)) {
				--xy_L.x;
			}
			...
			Vec2d xy_R = {adj_xy.x + offs_R.x, adj_xy.y + offs_R.y};
			size_t pos_R = static_cast<size_t>(xy_R.y * sz + xy_R.x);
			if (movable_map(Direction::Right).test(pos_R)) {
				--xy_R.x;
			}
			...
			auto adj_start_type = movable_map(Direction::Left).test(adj_pos)
			...
			if (dir_is_UD(dir)) {
				for (int j = num_boxes - 1; j >= 0; --j) {
					Vec2d current_xy = static_cast<Box_wide*>(box_ptr_v[j])
					                       ->position();

					Vec2d back_xy = {current_xy.x - offs.x,
					                 current_xy.y - offs.y};
					size_t current_pos = current_xy.y * sz + current_xy.x;
					size_t back_pos = back_xy.y * sz + back_xy.x;

					movable_map(Direction::Left).reset(back_pos);
					movable_map(Direction::Left).set(current_pos);
					movable_map(Direction::Right).set(current_pos + 1);
					movable_map(Direction::Right).reset(back_pos + 1);
				}
			} else {
				...
				if (dir_is_L(dir)) {
					for (int i = 0; i < num_boxes; ++i) {
						...
						size_t p_R = p_L + 1;
						movable_map(Direction::Left).reset(p_L);
						movable_map(Direction::Left).set(p_L - 1);
						movable_map(Direction::Right).reset(p_R);
						movable_map(Direction::Right).set(p_L);
					}
				} else {
					for (int i = 0; i < num_boxes; ++i) {
						size_t p_L = start_pos - 1 + i * 2;
						size_t p_R = p_L + 1;
						movable_map(Direction::Left).reset(p_L);
						movable_map(Direction::Left).set(p_R);
						movable_map(Direction::Right).reset(p_R);
						movable_map(Direction::Right).set(p_R + 1);
					}
				}
			}
			...
			for (size_t i = 0; i < box_ptr_v.size(); ++i) {
				...
				if (dir_is_UD(dir)) {
					for (auto* stat_L : stationary_adj_L) {
						update_adj(*stat_L, Direction::Right);
					}
					for (auto* stat_R : stationary_adj_R) {
						update_adj(*stat_R, Direction::Left);
					}
				}
				...
				if (dir_is_UD(dir)) {
					const auto& adj_inv = (dir == Direction::Up) ? b.adj_D
					                                             : b.adj_U;
...
{
	return (movable_map(Direction::Left).test(pos)
	        || movable_map(Direction::Right).test(pos)
	        || stuck_map(Direction::Left).test(pos)
	        || stuck_map(Direction::Right).test(pos));
}
...
aoc24::Dyn_bitset& aoc24_15::Box_grid_wide::movable_map(Direction dir) const
{
	throw_if_no_map();
...
aoc24::Dyn_bitset& aoc24_15::Box_grid_wide::stuck_map(Direction dir) const
{
	throw_if_no_map();
	if (dir == Direction::Left) {
		return *stuck_L_;
...
void aoc24_15::Box_grid_wide::print_map(std::ostream& ostr)
{
	...
	for (size_t y = 0; y < sz_y; ++y) {
		for (size_t x = 0; x < sz; ++x) {
			...
			bool has_mvl = movable_map(Direction::Left).test(i);
			bool has_mvr = movable_map(Direction::Right).test(i);
			bool has_stl = stuck_map(Direction::Left).test(i);
			bool has_str = stuck_map(Direction::Right).test(i);
			char c{};
...
long long aoc24_15::Box_grid_wide::sum_of_all_box_coordinates() const
{
	...
	print_->reset();
	(*print_) |= movable_map(Direction::Left);
	(*print_) |= stuck_map(Direction::Left);
...
void aoc24_15::Box_grid_wide::add_box(int pos_x, int pos_y)
{
	...
	movable_map(Direction::Left).set(pos);
	movable_map(Direction::Right).set(pos + 1);
...
aoc24_15::Adjacent_tile& aoc24_15::Box_grid_wide::adj_from_dir(Box& box,
                                                               Direction dir,
                                                               bool invert)
{
	if (invert) {
...
void aoc24_15::Box_grid_wide::update_adj(Box& box, Direction dir)
{
	Vec2d offs = dir_to_offset(dir);
	offs.x *= (dir == Direction::Right) ? 2 : 1; // Boxes are two tiles wide
	auto& box_s = static_cast<Box_wide&>(box);
	...
	std::vector<Vec2d> adj_xy_v = {adj_xy};
	if (dir_is_UD(dir)) {
		// Up / Down means there are two adjacent tiles
		adj_xy_v.emplace_back(Vec2d{adj_xy_v.front().x + 1, adj_xy_v.front().y});
	}

	for (int i = 0; i < static_cast<int>(adj_xy_v.size()); ++i) {
		Adjacent_subtile* subtile_ptr = nullptr;
		if (i == 0) {
			subtile_ptr = dir == Direction::Right ? tile.right_tile.get()
			                                      : tile.left_tile.get();
		} else {
		...
		const Vec2d& xy = adj_xy_v[i];
		const size_t pos = adj_xy_v[i].y * sz + adj_xy_v[i].x;
		if (is_on_box_map(pos)) {
			bool is_mov_L = movable_map(Direction::Left).test(pos);
			if (is_mov_L || movable_map(Direction::Right).test(pos)) {
				int offs_x = 0;
			...
			} else if (stuck_map(Direction::Left).test(pos)) {
...
void aoc24_15::Box_grid_wide::mark_as_stuck(Box& b)
{
	...
	stuck_map(Direction::Left).set(box_s.pos_.y * size() + box_s.pos_.x);
	stuck_map(Direction::Right).set(box_s.pos_.y * size() + box_s.pos_.x + 1);
	movable_map(Direction::Left).reset(box_s.pos_.y * size() + box_s.pos_.x);
	movable_map(Direction::Right).reset(box_s.pos_.y * size() + box_s.pos_.x + 1);
	for (int i = 0; i < 4; ++i) {
		...
		for (int j = 0; j < (dir_is_UD(adj_dir) ? 2 : 1); ++j) {
			const bool use_R = (j == 1 || adj_dir == Direction::Right);
                                                  ...
                                                  Direction dir) const
{
	...
	if (dir_is_UD(dir)) {
		return (
		    stuck_map(Direction::Left).test(adj_xy.y * size() + adj_xy.x)
		    || stuck_map(Direction::Left).test(adj_xy.y * size() + adj_xy.x + 1)
		    || stuck_map(Direction::Right).test(adj_xy.y * size() + adj_xy.x)
		    || stuck_map(Direction::Right)
		           .test(adj_xy.y * size() + adj_xy.x + 1));
	}
	return (stuck_map(Direction::Left).test(adj_xy.y * size() + adj_xy.x)
	        || stuck_map(Direction::Right).test(adj_xy.y * size() + adj_xy.x));
}
...
aoc24_15::Is_stuck aoc24_15::Box_grid_wide::is_stuck(const Box& b) const
{
	...
	s.up = (box_s.adj_U.left_tile->type == ATT::Wall
	        ...
	        || Box_grid_wide::adj_is_on_stuck_map(box_s, Direction::Up));
	s.right = (box_s.adj_R.right_tile->type == ATT::Wall
	           || box_s.adj_R.right_tile->type == ATT::Box_stuck_left
	           || Box_grid_wide::adj_is_on_stuck_map(box_s, Direction::Right));
	...
	s.left = (box_s.adj_L.left_tile->type == ATT::Wall
	          ...
	          || Box_grid_wide::adj_is_on_stuck_map(box_s, Direction::Left));
	return s;
}
    ...
    int pos_x, int pos_y, Direction dir)
{
	...
	Vec2d adj_xy = {pos_x + offs.x, pos_y + offs.y};
	if (is_oob(adj_xy.x, adj_xy.y)) {
		return {};
	}

	if (movable_map(Direction::Right).test(adj_xy.y * sz + adj_xy.x)) {
		--adj_xy.x;
	}
	std::stack<Adjacent_subtile> adj_subtiles;
	std::set<Vec2d> cached_boxes;
	...
	while (!found_wall_or_stuck && !box_stack.empty()) {
		...
		auto& current_box_w = *static_cast<Box_wide*>(current_box_ptr);
		box_stack.pop();
...
void aoc24_15::Box_grid_wide::throw_if_occupied(size_t pos) const
{
	if (walls_map().test(pos) || walls_map().test(pos + 1)
	    || movable_map(Direction::Left).test(pos)
	    || movable_map(Direction::Left).test(pos + 1)
	    || movable_map(Direction::Right).test(pos)
	    || movable_map(Direction::Right).test(pos + 1)
	    || stuck_map(Direction::Left).test(pos)
	    || stuck_map(Direction::Left).test(pos + 1)
	    || stuck_map(Direction::Right).test(pos)
	    || stuck_map(Direction::Right).test(pos + 1)) {
...
bool aoc24_15::Box_grid_wide::boxes_match_maps()
{
	for (auto& b : box_) {
		for (int i = 0; i < 4; ++i) {
			...
			Vec2d adj_xy_L = {b.position().x + offs.x, b.position().y + offs.y};
			Vec2d adj_xy_R = {adj_xy_L.x + 1, adj_xy_L.y};
...
bool aoc24_15::Box_grid_wide::dir_is_UD(const Direction dir) const
{
	return (dir == Direction::Up || dir == Direction::Down);
}

bool aoc24_15::Box_grid_wide::dir_is_L(const Direction dir) const
{
	return dir == Direction::Left;
}

bool aoc24_15::Box_grid_wide::dir_is_R(const Direction dir) const
{
	return dir == Direction::Right;
}

bool aoc24_15::Box_grid_wide::dir_is_UDL(const Direction dir) const
{
	return (dir_is_UD(dir) || dir_is_L(dir));
}

bool aoc24_15::Box_grid_wide::dir_is_UDR(const Direction dir) const
{
    ...
    Adjacent_subtile::Type t) const
{
	...
	if (t == ATT::Box_movable_left) {
		map = &movable_map(Direction::Left);
	} else if (t == ATT::Box_movable_right) {
		map = &movable_map(Direction::Right);
	} else if (t == ATT::Box_stuck_left) {
		map = &stuck_map(Direction::Left);
	} else if (t == ATT::Box_stuck_right) {
		map = &stuck_map(Direction::Right);
	} else {
...
{
	return (walls_map().test(pos) + movable_map(Direction::Left).test(pos)
	        + stuck_map(Direction::Left).test(pos)
	        + movable_map(Direction::Right).test(pos)
	        + stuck_map(Direction::Right).test(pos));
}

bool aoc24_15::Box_grid_wide::on_no_map(const Vec2d& xy,
    ...
    const bool stuck) const
{
	...
	if (dir == Direction::Left) {
		...
		throw std::invalid_argument("Direction to determine invalid "
		                            "side may only be Left / Right");
	}
	if (is_invalid) {
		std::cerr << "Expected " << (dir == Direction::Left ? "left" : "right")
		          << " part of " << (stuck ? "STUCK " : "")
...
bool aoc24_15::Box_grid_wide::unused_subtile_is_unknown(
    const Vec2d& pos, const Adjacent_tile_wide& tile, const Direction dir) const
{
	if (dir == Direction::Left) {
		...
		throw std::invalid_argument("Direction to determine valid unused "
    ...
    const bool found_right)
{
	...
	const Vec2d mod_adj_xy_R = dir_is_R(dir) ? adj_xy_L : adj_xy_R;
