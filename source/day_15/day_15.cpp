...
const std::string& debug_output_filename_p1_new()
{
	static const std::string p{"aoc24_15_p_1_debug_new.txt"};
	return p;
}
const std::string& debug_output_filename_p2_new()
{
	static const std::string p{"aoc24_15_p_2_debug_new.txt"};
	return p;
}
const std::string& debug_output_filename_compare()
{
	static const std::string p{"aoc24_15_p_2_debug_compare.txt"};
	return p;
}
} // namespace
...
try {
	...
	// Refactor
	Char_grid char_grid_refactor(chars, sz);
	Vec2d spawn_xy;
	std::vector<Vec2d> box_xy_v;
	extract_spawn_and_box_positions(char_grid_refactor, spawn_xy, box_xy_v);

	auto bitset_p1_refactor
	    = (pz.is_testing()) ? aoc24::Bitset_factory::create<sz_test * sz_test>()
	                        : aoc24::Bitset_factory::create<sz_inp * sz_inp>();

	Simple_box_factory sf;
	Warehouse p1_wh{sz, sz, std::move(bitset_p1_refactor), char_grid_refactor};
	...
	long long p1_sum{move_robot_and_get_sum_of_coordinates_new(
	    p1_wh, moves, enable_debug, debug_output_filename_p1_new())};

	pz.file_answer(1, "Sum of coordinates", p1_sum);

	Char_grid char_grid_refactor_wide{new_wider_grid(char_grid_refactor)};
	...
	auto bitset_p2_refactor
	    = (pz.is_testing())
	          ...
	          : aoc24::Bitset_factory::create<sz_inp * sz_inp * 2>();
	Rect_box_factory rf;
	Warehouse p2_wh{sz * 2,
	                sz,
	                std::move(bitset_p2_refactor),
	                char_grid_refactor_wide};
	for (const auto& xy : box_xy_v) {
		p2_wh.add_box(rf, xy.x * 2, xy.y);
	}
	p2_wh.spawn_robot(spawn_xy.x * 2, spawn_xy.y);
	long long p2_sum{move_robot_and_get_sum_of_coordinates_new(
	    p2_wh, moves, enable_debug, debug_output_filename_p2_new())};
...
long long aoc24_15::move_robot_and_get_sum_of_coordinates_new(
    Warehouse& grid,
    std::queue<aoc24::Direction> moves,
    bool enable_debug,
    const std::string debug_output_filename)
...
aoc24::Char_grid aoc24_15::new_wider_grid(const aoc24::Char_grid& reference_grid)
{
